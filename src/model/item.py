...
class Item(Base, db.Model):
    ...
    def __init__(self, name, image, description, price, item_status_id, person, item_type_id):
        self.name = name
        self.image = image
    ...
    def get_all_active_items_by_item_type_id(cls, item_type_id):
        item_status_id = ItemStatus.get_id_by_name("Available")
        return db.session.query(cls).filter(cls.item_type_id == item_type_id, cls.item_status_id == item_status_id).all()
